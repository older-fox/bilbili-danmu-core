...
const ROOM_ID = 7734200;//全局变量-房间号
...
function queryAllVideoInfoTimer() {// NOTE: 该计时器每十分钟更新一次所有视频的播放量情况
  onTimer.scheduleJob('* 0,10,20,30,40,50 * * * *',async ()=>{
    ...
    if (videoList != -1) {
      insert2Databse.insertVideoViewData(JSON.parse(videoList));// NOTE: 将视频播放量信息加入到数据库
      insert2Databse.insertVideoIndex(JSON.parse(videoList));// NOTE: 在索引表中添加或更新视频bvid信息
    ...
    let TimeString = DateInfo.toLocaleTimeString();
        ...
        const AiMessage = await aiReply.GetAiReply(msg.info[2][0],msg.info[1]);
          ...
          console.log('['+colors.red(TimeString)+']'+colors.brightGreen(msg.info[2][1])+':'+colors.brightMagenta(msg.info[1]));
			...
			//天选之人开始的完整信息
			console.log(msg);
      insert2Databse.insertAnchorLot(msg);
