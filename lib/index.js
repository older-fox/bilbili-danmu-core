...
const aiReply = require('./ai_chat.js')
const colors = require('colors');
const onTimer = require('node-schedule');
var online = 0;//全局变量-直播在线人数
let uid = 0;//全局变量-主播UID
let last_update = 0;//全局变量-上次获取粉丝列表的时间
...
async function getBasicInfo() {// NOTE: 尝试获取房间信息
  ...
  if (uid === -1){
    ...
    //queryAllVideoInfoTimer();// NOTE: 拉起定时任务-获取所有视频的相关信息
    main();// NOTE: 拉起主程序及容器
  }
}


function queryAllVideoInfoTimer() {// NOTE: 该计时器每十分钟更新一次所有视频的播放量情况
  onTimer.scheduleJob('* 0,10,20,30,40,50 * * * *',async ()=>{
    let DateInfo = new Date();
    let TimeString = DateInfo.toLocaleTimeString();
    var videoList =  await updateRoomInfo.GetFullVideos(uid,50);
    if (videoList !== -1) {
      await insert2Databse.insertVideoViewData(JSON.parse(videoList));// NOTE: 将视频播放量信息加入到数据库
      await insert2Databse.insertVideoIndex(JSON.parse(videoList));// NOTE: 在索引表中添加或更新视频bvid信息
      console.log('['+colors.red(TimeString)+']'+colors.brightMagenta('投稿信息更新:完成更新 ')+colors.brightRed(JSON.parse(videoList).length)+colors.brightMagenta(' 条视频信息'));
    }
  }
)} 
...
function getNewFollowerTimer() {// NOTE: 获取新关注的粉丝表
  onTimer.scheduleJob('30 * * * * *',async ()=>{
    if (last_update === 0){last_update = Math.round(Date.now() /1000);}
	  let newFollowerList = await updateRoomInfo.GetNewFollower(uid, last_update);
...
function infoGetTimer() {//定时获取房间的粉丝数及实时观众人数
  ...
  let DateInfo = new Date();
  let TimeString = DateInfo.toLocaleTimeString();
  let followerCount = await updateRoomInfo.GetFollowerCount(ROOM_ID);
  if (followerCount === -1 ){return;}//若查询被屏蔽，则取消操作,防止插入无效数据
  await insert2Databse.insertFullTimeRecord(online,followerCount);// NOTE: 记录每分钟的粉丝及观众数变化
  console.log('['+colors.red(TimeString)+']'+colors.brightMagenta('主动信息更新:粉丝数 ')+colors.brightYellow(followerCount)+colors.brightMagenta(' ,当前房间人气 ')+colors.brightRed(online));
  });
}
...
function main() {
    ...
    let TimeString = DateInfo.toLocaleTimeString();
		switch(msg.cmd){
			...
			//收到普通弹幕
			//console.log(msg);
			await insert2Databse.insertDanmu(msg);
        const AiMessage = await aiReply.GetAiReply(msg.info[2][0],msg.info[1]);
				if (AiMessage !== "请求出错") {
          console.log('['+colors.red(TimeString)+']'+colors.brightGreen(msg.info[2][1])+':'+colors.brightMagenta(msg.info[1])+colors.brightGreen("     <===Ai:")+colors.brightYellow(AiMessage));
        }else {
          console.log('['+colors.red(TimeString)+']'+colors.brightGreen(msg.info[2][1])+':'+colors.brightMagenta(msg.info[1]));
			...
			case 'INTERACT_WORD':
			//用户进入直播间消息
			await insert2Databse.insertEntry(msg);
			...
			//礼物被发送后的消息
			//console.log(msg);
			await insert2Databse.insertGift(msg);
			...
			//房间信息更新
				...
				await insert2Databse.insertData(msg, online);
			break;

			case 'SUPER_CHAT_MESSAGE':
			//醒目留言消息
			    await insert2Databse.insertSuperchat(msg);
			...
			//天选之人开始的完整信息
			console.log(msg);
			await insert2Databse.insertAnchorLot(msg);
			...
			//上舰长的消息
			case 'GUARD_BUY':
				await insert2Databse.insertGuardbuy(msg);
					switch (msg.data.guard_level){
					case 1:
