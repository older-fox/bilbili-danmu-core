...
async function insertSuperchat (info){
	const timestring = Date.now() /1000;
	const sql = "INSERT INTO `superchat_info` (`Id`, `uid`, `username`, `message`, `price`, `send_time`) VALUES (NULL, '"+info.data.uid+"', '"+info.data.user_info.uname+"', '"+info.data.price+"', '"+info.data.message+"', '"+timestring+"')";
	await pool.query(sql);
}
