...
async function insertSuperchat (info){
	...
	const sql = "INSERT INTO `superchat_info` (`Id`, `uid`, `username`, `message`, `price`, `send_time`) VALUES (NULL, '"+info.data.uid+"', '"+info.data.user_info.uname+"', '"+message+"', '"+info.data.price+"', '"+timestring+"')";
	await pool.query(sql,function(err){if (err){console.log(err.message);}});
}
...
//更新其所佩戴的勋章信息
async function updateTitle (uid,username,titlename,title_level,title_owner,title_owner_room){
	const timestring = Date.now() /1000;
	const sql = "INSERT INTO `entry_info` (`Id`, `uid`, `username`, `entry_time`) VALUES (NULL, '"+info.data.uid+"', '"+info.data.uname+"', '"+timestring+"')";
