...
async function insertSuperchat (info){
	const timestring = Date.now() /1000;
	let Msg = info.data.message.replace(/'/g, "");
	const sql = "INSERT INTO `superchat_info` (`Id`, `uid`, `username`, `message`, `price`, `send_time`) VALUES (NULL, '"+info.data.uid+"', '"+info.data.user_info.uname+"', '"+info.data.message+"', '"+Msg+"', '"+timestring+"')";
	await pool.query(sql);
}
...
async function insertGuardbuy (info){
	const timestring = Date.now() /1000;
	let Msg = info.data.message.replace(/'/g, "");
	const sql = "INSERT INTO `superchat_info` (`Id`, `uid`, `username`, `guard_level`, `price`, `num`, `send_time`) VALUES (NULL, '"+info.data.uid+"', '"+info.data.username+"', '"+info.data.guard_level+"', '"+info.data.price+"', '"+info.data.num+"', '"+timestring+"')";
	await pool.query(sql);
}
